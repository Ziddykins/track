...
sub whois_signal {
	...
	if ($data =~ /channels/) {
		...
		$sth = $dbh->prepare("SELECT `channels` FROM $sql_table WHERE unique_id = ?")
...
sub track_options {
	...
	if ($commands[0] eq 'sql' && !defined($commands[1])) {
		if (check_sql_info()) {
			...
			Irssi::print("%RSQL mode cannot be enabled until required information has been supplied via 'set sql'");
